...
async def _(session: CommandSession):
    if not Player(session).alive:
        await session.send('你已经死亡，请复活后再试。', at_sender=True)
    else:
        while 1:
            try:
                if not session.state.setdefault('stack', [NoviceVillage(session)]):
                    session.state['stack'].append(NoviceVillage(session))
                await session.state['stack'][-1].run()
            except Finish:
